...
def plot_rejections(accepted, rejected,run_num,options):
	...
	gs.update(hspace=0,wspace=0)
	ax = fig.add_subplot(gs[0:100,0:100])
	cax1 = ax.quiver(accepted['tar_ra'],accepted['tar_dec'], accepted_offset_ra, accepted_offset_dec,color='dodgerblue',width=0.003,label='accepted')
	cax2 = ax.quiver(rejected['tar_ra'],rejected['tar_dec'], rejected_offset_ra, rejected_offset_dec,color='r',width=0.003,label='rejected')
	ax.set_xlabel("Distance from pointing centre / degrees")
	ax.set_ylabel("Distance from pointing centre / degrees")
	ax.set_title("Source position offsets / arcsec")
